...
export default function BeritaPage() {
  ...
  const banner = identitas.banner_berita || "";
      ...
      >
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            <Newspaper className="h-8 w-8" style={{ color: "rgba(255,255,255,0.7)" }} />
            <h1 className="text-3xl font-bold text-white sm:text-4xl">
              {current === "id" ? "Berita" : current === "en" ? "News" : "新闻"}
            </h1>
          </div>
          <p className="mt-3 max-w-2xl text-lg" style={{ color: "rgba(255,255,255,0.8)" }}>
            {current === "id"
              ? `Informasi terkini${namaFakultas ? ` dari ${namaFakultas}` : ""}`
              : current === "en"
                ? `Latest information${namaFakultas ? ` from ${namaFakultas}` : ""}`
                : namaFakultas ? `来自${namaFakultas}的最新信息` : "最新信息"}
          </p>
        </div>
      </section>
