...
import { ProdiCard } from "@/components/features/prodi/ProdiCard";
...
export default function ProdiPage() {
  ...
  const banner = identitas.banner_prodi || "";
    ...
    async function fetchProdi() {
      try {
        ...
        if (result.success && result.data) {
          ...
          setError(result.error ?? "Gagal memuat data program studi");
        }
      } catch {
      ...
      >
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3">
            <GraduationCap className="h-8 w-8" style={{ color: "rgba(255,255,255,0.7)" }} />
            <h1 className="text-3xl font-bold text-white sm:text-4xl">
              {current === "id" ? "Program Studi" : current === "en" ? "Study Programs" : "专业"}
            </h1>
          </div>
          <p className="mt-3 max-w-2xl text-lg" style={{ color: "rgba(255,255,255,0.8)" }}>
            {current === "id"
              ? `Program studi${namaFakultas ? ` ${namaFakultas}` : ""}`
              : current === "en"
                ? `Study programs${namaFakultas ? ` of ${namaFakultas}` : ""}`
                : namaFakultas ? `${namaFakultas}的专业` : "专业"}
          </p>
        </div>
      </section>
        ...
        {loading ? (
          <div className="flex items-center justify-center py-20">
            <div
              ...
              style={{
                borderRightColor: mounted ? theme.primaryHex : "#2563eb",
                borderBottomColor: mounted ? theme.primaryHex : "#2563eb",
                borderLeftColor: mounted ? theme.primaryHex : "#2563eb",
